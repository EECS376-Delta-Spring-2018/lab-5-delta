...
void DesStatePublisher::initializePublishers() {
    ...
    lidar_alarm_subscriber_ = nh_.subscribe("lidar_alarm", 1, &DesStatePublisher::alarmCallback, this);
}
void DesStatePublisher::alarmCallback(const std_msgs::Bool& alarm_msg) 
{ 
  g_lidar_alarm = alarm_msg.data;
  ROS_INFO("LIDAR ALARm IN FUNCTION=%d",g_lidar_alarm);
...
void DesStatePublisher::pub_next_state() {
    ...
    ROS_INFO("LIDAR ALARM GLOBAL: %d", g_lidar_alarm);
    if (g_lidar_alarm){
        ROS_WARN("Lidar Alarm received");
    ...
    switch (motion_mode_) {
        ...
        case LIDAR_STOPPED: //this state must be reset by a service
            ROS_INFO("LIDAR ALARM GLOBAL: %d", g_lidar_alarm);
            if(g_lidar_alarm){
                desired_state_publisher_.publish(halt_state_);
